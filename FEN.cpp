...
void FEN::Load(const std::string& fen, ChessBoard& board) {
    ...
    LoadHalfMoves       ((stream >> token, token), board);
    LoadFullMoves       ((stream >> token, token), board);
}
...
void FEN::LoadHalfMoves(std::string half_moves, ChessBoard& board) {
...
void FEN::LoadFullMoves(std::string full_moves, ChessBoard& board) {
